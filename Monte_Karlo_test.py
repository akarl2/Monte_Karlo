...
def simulate(a, b, rt, Samples, eor, a_mass, b_mass, PRGk, SRGk, CGRk, emr, emo):
    ...
    if rt.name != PolyCondensation:
        ...
        sum_of_composition = sum(composition)
        try:
            composition = [composition[x:x + len(a.comp)] for x in range(0, len(composition), len(a.comp))]
            composition_tuple = [tuple(l) for l in composition]
        ...
        Amine_weight_loss = amine_ct * rt.wl
        Acid_weight_loss = acid_ct * rt.wl
        #OH_weight_loss = alcohol_ct * rt.wl
        Amine_weight_after = sum_of_composition - Amine_weight_loss
        Acid_weight_after = sum_of_composition - Acid_weight_loss
        #OH_weight_after = sum_of_composition - OH_weight_loss
        possible_weight_loss = min(Amine_weight_loss, Acid_weight_loss)
        mass_after = sum_of_composition - possible_weight_loss
        fraction_loss = (1 - (emo_a / possible_weight_loss)) * possible_weight_loss
        when_to_stop = sum_of_composition - fraction_loss
        temp_rxn_sum = sum_of_composition

        #runs the reaction
        while temp_rxn_sum > when_to_stop:
